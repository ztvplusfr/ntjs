...
import { Send, X, AlertCircle, CheckCircle, Film, Clock, AlertTriangle } from 'lucide-react'
...
export default function MovieRequestModal({ isOpen, onClose, movieTitle, movieId }: MovieRequestModalProps) {
  ...
  // Vérifier si l'utilisateur peut soumettre une demande pour ce film
  useEffect(() => {
    if (!session?.user?.id) return

    const checkSubmissionLimit = () => {
      const lastSubmission = localStorage.getItem(`movie_request_${session.user.id}_${movieId}`)
  ...
  // Pré-remplir le message avec les informations du film
  useEffect(() => {
    if (isOpen && movieTitle) {
      setMessage(`Bonjour, je souhaite demander l'ajout du film "${movieTitle}" (ID: ${movieId}) sur la plateforme. Pourriez-vous rendre les vidéos disponibles ? Merci !`)
    }
  }, [isOpen, movieTitle, movieId])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!canSubmit) {
      setError('Veuillez attendre avant de faire une nouvelle demande pour ce film')
      return
    }
    ...
    try {
      ...
      formData.append('type', 'movie_request')
      ...
      if (response.ok) {
        // Enregistrer la demande pour ce film
        if (session?.user?.id) {
          localStorage.setItem(`movie_request_${session.user.id}_${movieId}`, new Date().toISOString())
        }
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-white font-bold text-lg">Demander ce film</h3>
          ...
          <div className="px-6 py-4 bg-cyan-950/20 border-b border-cyan-500/20">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-cyan-400 text-xs">ID TMDB: {movieId}</p>
              </div>
            </div>
          </div>
            ...
            <div className="px-6 py-6 bg-gray-800/30 border-b border-white/10">
              <div className="text-center">
                ...
                <p className="text-gray-400 text-sm mb-6">
                  Vous devez être connecté via Discord pour faire une demande de film.
                </p>
                <Link
                  href="/auth/discord"
