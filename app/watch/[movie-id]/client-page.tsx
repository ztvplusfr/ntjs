...
export default function WatchPageClient({ params, searchParams }: WatchMovieClientProps) {
  ...
  if (videosData && videosData.videos && videosData.videos.length > 0) {
    ...
    if (!currentSelectedVideo) {
      ...
      currentSelectedVideo = videosWithIds.find((video: Video) =>
        video.quality === '1080p' && video.pub === 0 && video.lang === 'vostfr'
      ) ||
      videosWithIds.find((video: Video) =>
        video.quality === '1080p' && video.pub === 0
      ) ||
      videosWithIds.find((video: Video) => video.quality === '1080p') ||
      videosWithIds.find((video: Video) => video.lang === 'vostfr') ||
  ...
  return (
    <>
      <PageHead
        title={
          movie
            ? `${movie.title} en streaming HD gratuit sur ZTVPlus`
            : 'Regarder en streaming sur ZTVPlus'
        }
        description={
          movie?.overview
            ? movie.overview
            : movie
            ? `Regarder ${movie.title} en streaming HD gratuit sur ZTVPlus.`
            : 'Regarder vos films préférés en streaming HD gratuit sur ZTVPlus.'
        }
        keywords={
          movie
            ? [
                ...
                .filter(Boolean)
                .join(', ')
            : 'film, streaming, gratuit, VF, VOSTFR'
        }
        image={
          movie?.backdrop_path
            ? `https://image.tmdb.org/t/p/original${movie.backdrop_path}`
             ...
             <div className="mb-8">
               <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 w-full">
                 ...
                 <div className="lg:col-span-2 w-full">
                   <AuthGuard message="Connectez-vous pour accéder au streaming de ce film en haute qualité.">
